"""Train 4-model transformer fusion (ViTPose + three baselines).
...
from training.train_transformer import compute_accuracy_and_loss
...
def train_transformer_fusion_4(
    list_file: str,
    dataset_root: str,
    epochs: int = 10,
    max_iters: int = 20000,
    ...
    print(f"\n[4-model transformer] K={K} model_type={model_type}")
    print(f"d_model={d_model}, nhead={nhead}, enc={num_encoder_layers}, dec={num_decoder_layers}")
    ...
    print("\n" + "="*70, flush=True)
    print("Starting 4-model transformer fusion (requires vitpose p2d_cache)", flush=True)
    print("="*70, flush=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            if it % 10 == 0 or it == 1:
                ...
                print(
                    f"Epoch {epoch} iter {it:5d} | loss {loss:.4f} (coord {lc:.4f} + conf {lf:.4f}) "
        ...
        print(f"\n>>> Epoch {epoch} summary: loss={avg_loss:.4f}, acc={avg_acc*100:.1f}%, dist={avg_dist:.2f}px", flush=True)
        ...
        if val_list is not None:
            ...
            print(f"Val loss: {val_loss:.4f} | Val acc: {val_accuracy*100:.1f}% | dist {val_dist:.2f}px", flush=True)
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(description='Train 4-model transformer pose fusion')
