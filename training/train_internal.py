"""Training script for transformer-based *internal* fusion model.

This trains ``TransformerInternalPoseFusion`` / ``LightweightTransformerInternalFusion``
from ``transformer_internal_fusion.py``.

Data comes from the same cache-based pipeline as ``train_fusion.py`` and
``train_transformer_fusion.py``:
    - DEKR (hrnet) and OpenPose predictions are loaded from p2d_cache files
    - MediaPipe predictions are also loaded from p2d_cache

For DEKR and OpenPose we form simple per-joint feature vectors by concatenating
their (x, y) coordinates and confidence:
    feats_dekr     = concat([coords_hrnet, conf_hrnet], dim=-1)      -> (B, K, 3)
    feats_openpose = concat([coords_openpose, conf_openpose], dim=-1)-> (B, K, 3)

These are passed through ``JointFeatureEmbedding`` inside the model.  If you later
compute true internal embeddings (e.g. via hooks in the backbones), you can pass
those instead as long as you update ``dekr_feat_dim`` / ``openpose_feat_dim``.

MediaPipe continues to use its final joint predictions (coords+conf) exactly as
in ``TransformerPoseFusion``.
"""
...
def compute_accuracy_and_loss(
    ...
    """Compute loss and accuracy metrics (same formulation as transformer_fusion)."""
...
def _build_internal_features(
    coords_hrnet: torch.Tensor,
    conf_hrnet: torch.Tensor,
    coords_openpose: torch.Tensor,
    conf_openpose: torch.Tensor,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """Construct simple per-joint feature vectors from cached predictions.

    Currently we use [x, y, conf] per joint for DEKR and OpenPose.
    This yields feature_dim=3 for each model.
    """
    feats_dekr = torch.cat([coords_hrnet, conf_hrnet], dim=-1)
    feats_openpose = torch.cat([coords_openpose, conf_openpose], dim=-1)
    return feats_dekr, feats_openpose
...
def train_step(
    ...
    feats_dekr, feats_openpose = _build_internal_features(
        coords_hrnet, conf_hrnet, coords_openpose, conf_openpose
    )
    ...
    mask_any = (
        (mask_hr.squeeze(-1) + mask_op.squeeze(-1) + mask_mp.squeeze(-1)) > 0.0
    )
...
def train_transformer_internal_fusion(
    list_file: str,
    dataset_root: str,
    epochs: int = 10,
    max_iters: int = 20000,
    ...
    """Training loop for transformer-based *internal* fusion model."""
    device_t = torch.device(
        device if (device == "cpu" or torch.cuda.is_available()) else "cpu"
    )

    # Probe dataset to determine number of joints (K)
    ...
    if num_joints is None:
        ...
        if num_joints != K:
            print(
                f"Warning: requested num_joints={num_joints} differs from dataset K={K}; using K={K}"
            )
        used_num_joints = K
    ...
    print("\n" + "=" * 70, flush=True)
    print("Starting transformer INTERNAL fusion training...", flush=True)
    print("=" * 70, flush=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            if it % 10 == 0 or it == 1:
                ...
                print(
                    ...
                    f"(coord {loss_coords:.4f} + conf {loss_conf:.4f}) "
                    f"| acc {acc*100:5.1f}% | dist {dist_px:6.2f}px",
                    flush=True,
                )
        ...
        print(
            f"\n>>> Epoch {epoch} summary: loss={avg_loss:.4f}, "
            f"accuracy={avg_acc*100:.1f}%, dist={avg_dist:.2f}px",
            flush=True,
        )
        ...
        if val_list is not None:
            ...
            print(
                f"Val loss: {val_loss:.4f} | Val accuracy: {val_accuracy*100:.1f}% "
            ...
            if val_accuracy > best_val_accuracy:
                ...
                if save_dir is not None:
                    ...
                    print(
                        f"✓ New best checkpoint saved (accuracy={best_val_accuracy*100:.1f}%)",
                        flush=True,
                    )
            else:
                epochs_since_improve += 1
                print(
                    f"No improvement ({epochs_since_improve}/{early_stop_patience} epochs)",
                    flush=True,
                )
            ...
            if (
                ...
                print(
                    f"Early stopping triggered after {epochs_since_improve} "
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Train transformer-based INTERNAL pose fusion model"
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=1e-4,
        help="Learning rate (default: 1e-4 for transformer)",
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=0.05,
        help="Accuracy threshold (normalized coords)",
    )
    p.add_argument(
        "--lambda-conf", type=float, default=0.1, help="Confidence loss weight"
    )
    ...
    p.add_argument(
        "--pin-memory", action="store_true", help="Pin memory for DataLoader"
    )
    p.add_argument(
        "--persistent-workers", action="store_true", help="Use persistent workers"
    )
    ...
    train_transformer_internal_fusion(
        list_file=args.list,
